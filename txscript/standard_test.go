...
// newAddressPubKey returns a new btgutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) btgutil.Address {
	addr, err := btgutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new btgutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) btgutil.Address {
	addr, err := btgutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new btgutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) btgutil.Address {
	addr, err := btgutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the btgutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the btgutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := btgutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := btgutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := btgutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := btgutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := btgutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*btgutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*btgutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*btgutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*btgutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*btgutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
