...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a btgutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "12gpXQVcCL2qhTNQgyLVdCFG2Qs2px98nV"
	address, err := btgutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := btgutil.Hash160(pubKey.SerializeCompressed())
	addr, err := btgutil.NewAddressPubKeyHash(pubKeyHash,
	...
	lookupKey := func(a btgutil.Address) (*btcec.PrivateKey, bool, error) {
