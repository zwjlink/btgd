...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr btgutil.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr btgutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := btgutil.NewAddressPubKeyHash(
				btgutil.Hash160(pk), &chaincfg.TestNetParams)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := SignTxOutput(&chaincfg.TestNetParams,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*btcec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := btgutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := btgutil.NewAddressScriptHash(
				pkScript, &chaincfg.TestNetParams)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk1 := (*btcec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := btgutil.NewAddressPubKey(pk1,
				&chaincfg.TestNetParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			pk2 := (*btcec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := btgutil.NewAddressPubKey(pk2,
				&chaincfg.TestNetParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*btgutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
