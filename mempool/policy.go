...
const (
	...
	DefaultMinRelayTxFee = btgutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee btgutil.Amount) int64 {
	...
	if minFee < 0 || minFee > btgutil.MaxSatoshi {
		minFee = btgutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *btgutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee btgutil.Amount) bool {
...
func checkTransactionStandard(tx *btgutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee btgutil.Amount,
...
func GetTxVirtualSize(tx *btgutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
