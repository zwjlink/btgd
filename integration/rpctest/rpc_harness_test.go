...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt btgutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(btgutil.Amount(5 * btgutil.SatoshiPerBitcoin))
	...
	txid = genSpend(btgutil.Amount(500 * btgutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(btgutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*btgutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, btgutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := btgutil.Amount(250 * btgutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = btgutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := btgutil.Amount(50 * btgutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := btgutil.Amount(numMatureOutputs * 50 * btgutil.SatoshiPerBitcoin)
