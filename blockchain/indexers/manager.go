...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block *btgutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block *btgutil.Block, view *blockchain.UtxoViewpoint) error {
	...
	for i := len(m.enabledIndexes); i > 0; i-- {
		...
		for !chain.MainChainHasBlock(hash) {
			...
			err = m.db.Update(func(dbTx database.Tx) error {
				...
				block, err := btgutil.NewBlockFromBytes(blockBytes)
				if err != nil {
					return err
				}
...
func makeUtxoView(dbTx database.Tx, block *btgutil.Block, interrupt <-chan struct{}) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range block.Transactions() {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(btgutil.NewTx(originTx), 0)
		}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block *btgutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block *btgutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
