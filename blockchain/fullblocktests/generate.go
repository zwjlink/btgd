...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: btgutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := btgutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*btgutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, btgutil.NewTx(tx))
	}
...
func additionalCoinbase(amount btgutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee btgutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee btgutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee btgutil.Amount) *wire.MsgTx {
