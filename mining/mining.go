...
const (
	...
	MinHighPriority = btgutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr btgutil.Address) (*btgutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *btgutil.Tx, height int32) error {
...
func logSkippedDeps(tx *btgutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress btgutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*btgutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := btgutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := btgutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A btgutil.Block should use saved in the state to avoid
